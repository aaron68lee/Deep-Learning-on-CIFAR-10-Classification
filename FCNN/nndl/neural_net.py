...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #print(X.shape)
    #print(W1.T.shape)
    h1 = np.dot(X, W1.T) + b1 #X @ W1.T + b1 # hidden layer 1
    h2 = np.maximum(0, h1) # ReLU
    z2 = h2 @ W2.T + b2 # hidden layer 2
    exp_ = np.exp(z2 - np.max(z2, axis=1, keepdims=True))
    scores = exp_ / np.sum(exp_, axis=1, keepdims=True)
    ...
    class_probs = probs[np.arange(N), y]

    data_loss = -np.sum(np.log(class_probs)) / N
    ...
    score_copy = probs.copy()
    score_copy[np.arange(N), y] -= 1
    score_copy /= N

    grads['W2'] = (score_copy).T @ h2 + reg * W2
    grads['b2'] = np.sum(score_copy, axis=0)

    hidden = score_copy @ W2
    hidden[h2 <= 0] = 0

    grads['W1'] = (hidden).T @ X + reg * W1
  ...
  def predict(self, X):
    ...
    h1 = X @ W1.T + b1 # hidden 1
    h2 = np.maximum(0, h1) @ W2.T + b2 # ReLU and hidden 2
    exp_ = np.exp(h2 - np.max(h2, axis=1, keepdims=True))
    output_probs = exp_ / np.sum(exp_, axis=1, keepdims=True) # softmax
    
    y_pred = np.argmax(output_probs, axis=1)
    #print("prob:", output_probs)
    #print("pred y:", y_pred)
