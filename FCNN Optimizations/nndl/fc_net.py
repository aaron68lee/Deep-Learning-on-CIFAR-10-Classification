...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      layer_out, acache = affine_forward(layer_input, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
      ...
      if i != self.num_layers - 1:
        # apply batch norm
        if self.use_batchnorm:
          layer_out, bcache = batchnorm_forward(layer_out, self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], self.bn_params[i])
